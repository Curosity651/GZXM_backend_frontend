...
class AchievementWorkflowTest {
    ...
    void everyUnitActionHasAnExplicitAllowedOrRejectedTransition(String type) {
        ...
        if(Set.of("PAPER","PATENT").contains(type)) {
            allowed.put("PRE_APPROVED/REGISTER_EXTERNAL_SUBMISSION","EXTERNAL_SUBMITTED");allowed.put("EXTERNAL_SUBMITTED/START_FORMAL","FORMAL_DRAFT");
            allowed.put(("PAPER".equals(type)?"WAIT_PUBLICATION":"WAIT_GRANT")+"/SUBMIT_SUPPLEMENT","SUPPLEMENT_INITIAL");
            allowed.put("SUPPLEMENT_RETURNED/SUBMIT_SUPPLEMENT","SUPPLEMENT_INITIAL");
        }
        for(String state:List.of("DRAFT","PRE_INITIAL","PRE_FINAL","PRE_RETURNED","PRE_APPROVED","EXTERNAL_SUBMITTED","FORMAL_DRAFT","FORMAL_INITIAL",
                "FORMAL_FINAL","FORMAL_RETURNED","WAIT_PUBLICATION","WAIT_GRANT","SUPPLEMENT_INITIAL","SUPPLEMENT_FINAL","SUPPLEMENT_RETURNED","EFFECTIVE","UNKNOWN"))
            for(String action:List.of("SUBMIT_PRE_REVIEW","REGISTER_EXTERNAL_SUBMISSION","START_FORMAL","SUBMIT_FORMAL","SUBMIT_SUPPLEMENT","UNKNOWN")) {
    ...
    void approvalsReturnsAndFinalRecognitionFollowEachStage(String type) {
        ...
        assertThat(AchievementWorkflow.review("PRE_FINAL","APPROVE",type)).isEqualTo(Set.of("PAPER","PATENT").contains(type)?"PRE_APPROVED":"FORMAL_DRAFT");
        assertThat(AchievementWorkflow.review("FORMAL_FINAL","APPROVE",type)).isEqualTo("PAPER".equals(type)?"WAIT_PUBLICATION":"PATENT".equals(type)?"WAIT_GRANT":"EFFECTIVE");
