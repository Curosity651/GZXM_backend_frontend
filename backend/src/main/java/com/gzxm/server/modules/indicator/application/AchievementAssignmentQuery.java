...
public class AchievementAssignmentQuery {
    ...
    private final UnitAllocationMapper allocations;
    private final SecurityContextFacade security;
    public AchievementAssignmentQuery(TopicQueryService topics,TopicIndicatorMapper indicators,UnitAllocationMapper allocations,SecurityContextFacade security) {
        this.topics=topics; this.indicators=indicators; this.allocations=allocations; this.security=security;
    }
    public Assignment requireAssigned(long topicId,long nodeId,long definitionId) {
        ...
        if(allocations.effective(topicId,nodeId,user.unitId()).stream().noneMatch(row->row.indicatorDefinitionId().equals(Long.toString(definitionId)))
                || indicators.effective(topicId,nodeId).stream().noneMatch(row->row.indicatorDefinitionId().equals(Long.toString(definitionId))))
            throw BusinessException.conflict("ACHIEVEMENT_ALLOCATION_REQUIRED","请先下发当前单位该节点的基础指标");
