...
public final class AchievementWorkflow {
    ...
    public static final Set<String> EDITABLE=Set.of("DRAFT","PRE_RETURNED","FORMAL_DRAFT","FORMAL_RETURNED","WAIT_PUBLICATION","WAIT_GRANT","SUPPLEMENT_RETURNED");
    ...
    public static String action(String status,String action,String type) {
        ...
        if("REGISTER_EXTERNAL_SUBMISSION".equals(action) && "PRE_APPROVED".equals(status) && external(type)) return "EXTERNAL_SUBMITTED";
        if("START_FORMAL".equals(action) && "EXTERNAL_SUBMITTED".equals(status) && external(type)) return "FORMAL_DRAFT";
        ...
        if("SUBMIT_SUPPLEMENT".equals(action) && (("PAPER".equals(type) && "WAIT_PUBLICATION".equals(status))
                || ("PATENT".equals(type) && "WAIT_GRANT".equals(status)) || (external(type) && "SUPPLEMENT_RETURNED".equals(status)))) return "SUPPLEMENT_INITIAL";
    ...
    public static String review(String status,String decision,String type) {
        ...
        if("PRE_FINAL".equals(status)) return external(type)?"PRE_APPROVED":"FORMAL_DRAFT";
        if("FORMAL_FINAL".equals(status)) return "PAPER".equals(type)?"WAIT_PUBLICATION":"PATENT".equals(type)?"WAIT_GRANT":"EFFECTIVE";
        return "EFFECTIVE";
    }
    ...
    private static boolean external(String type) {return "PAPER".equals(type)||"PATENT".equals(type);}
}
