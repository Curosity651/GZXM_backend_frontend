  ...
  return <div className="role-management-page">
    ...
    <Modal title={`编辑角色权限${editing ? ` · ${editing.name}` : ''}`} open={open} width={900} onCancel={() => { setOpen(false); editForm.resetFields(); }} onOk={() => void save()} confirmLoading={saving}>
      <Form form={editForm} layout="vertical">
        ...
        <Form.Item name="pagePermissions" label="页面权限" rules={[{ required: true, message: '请至少选择一个页面' }]}><Checkbox.Group style={{ width: '100%' }}><Row gutter={[8, 12]}>{Object.entries(groupedPages).map(([group, items]) => <Col span={12} key={group}><Card size="small" title={group}>{items.map((item) => <div key={item.code}><Checkbox value={item.code} disabled={permissionLocked(item)}>{item.name}{permissionLocked(item) ? '（外部单位禁止）' : ''}</Checkbox></div>)}</Card></Col>)}</Row></Checkbox.Group></Form.Item>
        <Form.Item name="actionPermissions" label="操作权限"><Checkbox.Group style={{ width: '100%' }}><Row gutter={[8, 12]}>{Object.entries(groupedActions).map(([group, items]) => <Col span={12} key={group}><Card size="small" title={group}>{items.map((item) => <div key={item.code}><Checkbox value={item.code} disabled={permissionLocked(item)}>{item.name}{permissionLocked(item) ? '（外部单位禁止）' : ''}</Checkbox></div>)}</Card></Col>)}</Row></Checkbox.Group></Form.Item>
      </Form>
    </Modal>
  </div>;
}
