...
function MockSelfFundedProjectPage() {
  ...
  const [editingProject, setEditingProject] = useState<SelfFundedProject | null>(null);
  ...
  const saveProject = async () => {
    ...
    try {
      if (editingProject) {
        state.updateSelfFundedProject(editingProject.id, values, user.id);
      } else {
      ...
      setModal(false); setEditingProject(null); form.resetFields();
      message.success(editingProject ? '项目基础信息已更新' : '配套自筹项目已创建，并已生成对应类型的归档清单快照');
    } catch (error) { message.warning(error instanceof Error ? error.message : '项目保存失败'); }
  };
  const cardActions = (project: SelfFundedProject) => [
    <Button key={`open-${project.id}`} type="link" onClick={() => { setSelected(project); setSelectedFolder(null); }}>进入归档</Button>,
    ...(editable && project.ownerUnitId === user.unitId ? [<Button key={`edit-${project.id}`} type="link" onClick={() => {
      setEditingProject(project); form.setFieldsValue(project); setModal(true);
    }}>编辑基础信息</Button>] : []),
  ];
  ...
  return <>
    ...
    {editable && editableTopics.length > 0 && <div className="archive-page-actions"><Button type="primary" icon={<FileAddOutlined />} onClick={() => {
      setEditingProject(null); form.resetFields(); form.setFieldsValue({ topicId: primaryTopicId, status: '筹备中' }); setModal(true);
    }}>新建自筹项目</Button></div>}
    ...
    <Modal title={editingProject ? '编辑配套自筹项目' : '新建配套自筹项目'} open={modal} onCancel={() => setModal(false)} onOk={() => void saveProject()} width={680}><Form form={form} layout="vertical"><Form.Item label="所属课题" name="topicId" rules={[{ required: true }]}><Select disabled={Boolean(editingProject)} options={editableTopics.map((item) => ({ label: `${item.code} ${item.name}`, value: item.id }))} /></Form.Item><Row gutter={16}><Col span={12}><Form.Item label="项目编号" name="code" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={12}><Form.Item label="项目类型" name="projectType" rules={[{ required: true }]}><Select disabled={Boolean(editingProject)} options={Object.keys(templateMap).map((item) => ({ label: item, value: item }))} /></Form.Item></Col></Row><Form.Item label="项目名称" name="name" rules={[{ required: true }]}><Input /></Form.Item><Row gutter={16}><Col span={12}><Form.Item label="项目负责人" name="principalName" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={12}><Form.Item label="实施单位"><Input value={state.units.find((item) => item.id === user.unitId)?.name} disabled /></Form.Item></Col></Row><Row gutter={16}><Col span={8}><Form.Item label="开始日期" name="startDate" rules={[{ required: true, message: '请选择开始日期' }]}><Input type="date" /></Form.Item></Col><Col span={8}><Form.Item label="结束日期" name="endDate" rules={[{ required: true, message: '请选择结束日期' }]}><Input type="date" /></Form.Item></Col><Col span={8}><Form.Item label="预算（万元）" name="budget"><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col></Row><Form.Item label="状态" name="status" initialValue="筹备中"><Select options={['筹备中', '实施中', '验收中', '已完成'].map((item) => ({ label: item, value: item }))} /></Form.Item></Form></Modal>
...
export function SelfFundedProjectPage() {
  return isRealApi() ? <RealSelfFundedPage /> : <MockSelfFundedProjectPage />;
}
