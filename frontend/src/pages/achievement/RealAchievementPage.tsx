...
import { CheckOutlined, EditOutlined, EyeOutlined, PlusOutlined, ReloadOutlined, RollbackOutlined, SendOutlined, UploadOutlined } from '@ant-design/icons';
...
export function RealAchievementPage() {
  ...
  const [filters, setFilters] = useState({ topicId: '', nodeId: '', status: '', indicatorDefinitionId: '', keyword: '', pendingForMe: false });
  ...
  return <>
    <Card className="achievement-filter-card" style={{ marginBottom: 16 }}><Row gutter={[14, 14]} align="middle">
      {(canInitial || canFinal) && <Col><Select style={{ width: 130 }} value={filters.pendingForMe} onChange={(value) => setFilters({ ...filters, pendingForMe: value })} options={[{ value: true, label: '待我处理' }, { value: false, label: '全部成果' }]} /></Col>}
      <Col><Select style={{ width: 210 }} allowClear placeholder="所属课题" value={filters.topicId || undefined} onChange={(value) => setFilters({ ...filters, topicId: value ?? '' })} options={topics.map((item) => ({ value: item.id, label: `${item.code} ${item.name}` }))} /></Col>
      <Col><Select style={{ width: 170 }} placeholder="考核节点" value={filters.nodeId || undefined} onChange={(value) => setFilters({ ...filters, nodeId: value })} options={nodes.map((item) => ({ value: item.id, label: item.name }))} /></Col>
      <Col><Select style={{ width: 170 }} allowClear placeholder="成果状态" value={filters.status || undefined} onChange={(value) => setFilters({ ...filters, status: value ?? '' })} options={Object.entries(statusNames).map(([value, label]) => ({ value, label }))} /></Col>
      <Col flex="auto"><Input allowClear placeholder="成果名称" value={filters.keyword} onChange={(event) => setFilters({ ...filters, keyword: event.target.value })} /></Col>
      <Col><Space><Button icon={<ReloadOutlined />} onClick={() => void loadRows()}>刷新</Button>{canSubmit && <Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>新增成果</Button>}</Space></Col>
    </Row></Card>
    {progress && <Row gutter={14} style={{ marginBottom: 16 }}><Col span={6}><Card><Statistic title="分配指标" value={Object.values(baseTotals).reduce((sum, value) => sum + value, 0)} /></Card></Col><Col span={6}><Card><Statistic title="已发起" value={progress.baseStages.initiated} /></Card></Col><Col span={6}><Card><Statistic title="正式成果" value={progress.baseStages.formal} /></Card></Col><Col span={6}><Card><Statistic title="已生效" value={progress.baseStages.effective} suffix={<Text type="secondary"> 项</Text>} /></Card></Col></Row>}
