...
export function AchievementQueryPage() {
  ...
  return <>
    <Row gutter={12} style={{ marginBottom: 16 }}>
      {[['指标总数', totals.target], ['已发起', totals.initiated], ['预审通过', totals.preApproved], ['已投稿/申请', totals.external], ['正式审批', totals.formal], ['补充阶段', totals.supplement], ['已完成', totals.effective]].map(([label, value]) => <Col flex="1 1 130px" key={String(label)}><Card><Statistic title={label} value={value} /></Card></Col>)}
    </Row>
    ...
    <Card>
      ...
      <Table rowKey="key" dataSource={rows} scroll={{ x: 1180 }} columns={[
        ...
        { title: '分配指标', dataIndex: 'target', width: 90 }, { title: '已发起', dataIndex: 'initiated', width: 80 }, { title: '预审通过', dataIndex: 'preApproved', width: 90 }, { title: '已投稿/申请', dataIndex: 'external', width: 110 }, { title: '正式审批', dataIndex: 'formal', width: 90 }, { title: '补充阶段', dataIndex: 'supplement', width: 90 }, { title: '已完成', dataIndex: 'effective', width: 80 },
