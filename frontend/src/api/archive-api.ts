...
export interface ApiArchiveFolder { id: string; topicId: string; unitId: string; ownerType: 'TOPIC_NATIONAL' | 'SELF_FUNDED'; ownerId: string; categoryName: string; name: string; required: boolean; requiredQuantity: number; custom: boolean; fileCount: number; completed: boolean; canDelete: boolean }
...
export const archiveApi = {
  ...
  addNationalFolder: (topicId: string, unitId: string, categoryName: string, name: string, required: boolean) => apiRequest<ApiArchiveFolder>(`/archive/national/topics/${topicId}/units/${unitId}/folders`, { method: 'POST', body: JSON.stringify({ categoryName, name, required }) }),
  ...
  addProjectFolder: (id: string, categoryName: string, name: string, required: boolean) => apiRequest<ApiArchiveFolder>(`/self-funded-projects/${id}/folders`, { method: 'POST', body: JSON.stringify({ categoryName, name, required }) }),
