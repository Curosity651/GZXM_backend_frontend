...
需要区分“带当前用户的数据范围查询”和“受信任内部身份事实查询”，避免认证加载用户时反向依赖尚未建立的 CurrentUser，造成循环。名称、Java 签名和 DTO 在第 3 步评审落实，不在本步骤假装已提供可注入 Bean。
...
| D04 | 五类 detail 为开放对象；成果应绑定本单位已下发目标的条件不完整 | 定义分类型、分阶段字段及枚举、日期、排序、长度；已生效归属不可随改 | B 与业务、A 审 | 6 |
| D05 | materialFileIds 无材料类型/版本；数据库同类同版本唯一，旧前端允许同类多文件；文件服务缺公开校验 | 评审结构化材料请求与响应、材料版本模型；A 提供可关联文件元数据及下载授权 | A/B，共享文件 C 审 | 6、7 |
