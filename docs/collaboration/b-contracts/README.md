...
以下保留原始提案供追溯；当前以第 7 步状态矩阵和 OpenAPI 为准。用户已确认五类均预审并在正式终审后计数；论文/专利继续完成第三轮补充材料双审且不撤销已有计数，所有退回重提均回初审。
...
第 7 步已确认：人才不跳过预审；所有阶段两级审批；退回重提回初审；审批中锁定；正式终审通过后计数，论文/专利补充审批不撤销已有计数。专项聚合由第 8 步处理。
