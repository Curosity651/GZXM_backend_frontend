...
- 成果：第 7 步按用户确认的规则补充 AchievementStatus 枚举，见[成果流程实现与验收](achievement-step7.md)。
...
### 6.2 成果流转评审表（原始提案，已由第 7 步确认方案替代）

以下保留原始提案供追溯；当前以第 7 步状态矩阵和 OpenAPI 为准。用户已确认五类均预审，论文/专利补充终审后计数，其他三类正式终审计数，所有退回重提均回初审。
...
第 7 步已确认：人才不跳过预审；所有阶段两级审批；退回重提回初审；审批中和生效后锁定；补充前论文/专利不计数。专项聚合仍留第 8 步。A/C 人工评审尚未代签。
...
| D03 | 用户已确认五类两级预审、论文/专利补充认定及退回重提规则 | 第 7 步已实现显式状态机和 OpenAPI 枚举；正式带材料闭环等待 A | 业务已确认；A/C 评审未完成 | 7 |
| D04 | 用户确认：有效成员、已下发基础指标（允许零）、归属不可改；detail 按现有表单，草稿允许缺项；阶段日期与材料口径已确认 | 第 6/7 步已落实草稿字段和阶段必填校验，预审不强制附件 | 用户业务确认已收到；A/C 评审未完成 | 6、7 |
...
| 9 联调 | 全部 | 前端真实 API、五角色操作、刷新持久化及跨模块查询一致；[B 前端交付与剩余依赖](achievement-step9.md)，完整 E2E 尚待 A 公共能力与 C 联调 |
