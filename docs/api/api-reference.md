...
| POST | `/users` | 创建用户并指定角色和联系人信息；课题单位账号由后端按用户名自动建立同名单位并一次性返回临时密码 | 系统管理员；单位内外属性由角色决定，同一单位只能有一个有效课题单位账号 |
...
| POST | `/users/{userId}/password:reset` | 重置密码、使旧会话失效并一次性返回临时密码 | 系统管理员 |
